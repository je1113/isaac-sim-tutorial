...
## 8. Replicator (합성 데이터) — `omni.replicator.core`
...
world = World(stage_units_in_meters=1.0)   # 반드시 먼저 생성 — 없으면 조명을 아무리 세게 줘도 새까맣게 렌더링됨
world.scene.add_default_ground_plane()
world.reset()

with rep.new_layer():
    dome_light = rep.create.light(light_type="Dome", intensity=1000.0)   # 정수 아니라 float로!
    camera = rep.create.camera(position=(0, 0, 5), look_at=(0, 0, 0))
    cube = rep.create.cube(semantics=[("class", "cube")])

    with rep.trigger.on_frame(num_frames=100, rt_subframes=4):
        with cube:
            rep.modify.pose(
        ...
        with dome_light:
            # 런타임 수정 시엔 네임스페이스 포함한 실제 USD 속성명을 써야 함
...
rep.orchestrator.run_until_complete()   # run()은 시작만 시키고 바로 리턴함 - standalone에선 이거 사용
```

기존 씬(예: Module 4의 `franka_scene.usd`)에 적용할 때는 `rep.get.prims(path_pattern="/World/Cube")`로 이미 존재하는 prim을 가져와서 `with` 블록에 넣으면 된다. Semantic label은 `rep.create.*`의 `semantics=` 인자 대신 [`isaacsim.core.utils.semantics.add_labels`](#6-semantic-label--isaacsimcoreutilssemantics)를 직접 써야 한다 (기존 prim이라 생성 시점 라벨링이 불가능).

출력물 구조:
- `rgb_NNNN.png` — 그대로 RGB 이미지
- `bounding_box_2d_tight_NNNN.npy` — structured array `(semanticId, x_min, y_min, x_max, y_max, occlusionRatio)`, **JSON이 아님**
- `bounding_box_2d_tight_labels_NNNN.json` — `{"0": {"class": "cube"}}` semanticId → 클래스 이름 매핑 (프레임별로 따로 저장됨)
- `semantic_segmentation_NNNN.png` — `colorize_semantic_segmentation=True`면 바로 시각화 가능한 컬러 PNG

> `World()` 없이 Replicator만 쓰면 렌더가 새까맣게 나오고, 조명 intensity를 정수로 만들면 나중에 float로 랜덤화할 때 타입 충돌이 난다 — 둘 다 겉보기엔 관련 없어 보이지만 실제로 자주 걸리는 함정 ([Module 6](m06_replicator/README.md#1-최소-replicator-스크립트)).
...
| `orchestrator.run()` 호출 직후 출력 폴더가 비어있음 | `run()`은 시작만 시키고 바로 리턴 (블로킹 안 함) | [Module 6](m06_replicator/README.md#1-최소-replicator-스크립트) |
| Replicator 렌더가 조명 세기 무관하게 계속 새까맣게 나옴 | `World()`를 안 만들어서 RTX 렌더 파이프라인이 초기화 안 됨 | [Module 6](m06_replicator/README.md#1-최소-replicator-스크립트) |
| `rep.modify.attribute`에서 `Type mismatch: expected 'int'` | 생성 시 정수 리터럴 사용 + 네임스페이스 없는 속성명 | [Module 6](m06_replicator/README.md#3-조명배경-랜덤화-확장) |

---
[Module 1](m01_usd_basics/README.md) · [Module 2](m02_python_api/README.md) · [Module 3](m03_physx/README.md) · [Module 4](m04_manipulation/README.md) · [Module 5](m05_sensors/README.md) · [Module 6](m06_replicator/README.md)
